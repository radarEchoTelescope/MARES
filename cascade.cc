...
double Cascade::Ne(double X, double r, double delta_r){
	return Ne(X,r,delta_r, fEnergy, fNp);
}

// Full N(e), applies to a cascade of Np particles with energy Ep per particle. 
double Cascade::Ne(double X, double r, double delta_r, double Ep, double Np){
	if (r < 0) {r = -r;} 
	return Np* N(X,Ep)* E_deposition/E_ionization * rho_ice * intwiv(r,delta_r);
}
...
// Density for beam of energy E and containing N particles.
...
double Cascade::Density(double X, double r, double delta_r, double E, double Np){
	...
	if (r < 0) {r = -r;}
	// if (X < 0) {X = 0;}
	dens = Ne(X,r,delta_r, E, Np) / (pi*(pow(r + delta_r,2) - pow(r,2)));
...
namespace {
	...
	*/
	double intwiv(double r, double delta_r, double s){    // [cm, cm, unitless]
		double intwiv = 0, imx = 100.0, step = delta_r/imx;
